...
<div class="page-header">
    ...
    <nav aria-label="breadcrumb">
        <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item active"> <a href="{{ route('penjualan.axios') }}">Penjualan (axios)</a></li>
    ...
    <script>
        $(document).ready(function () {
            ...
            function pembayaran() {
                ...
                axios.post('{{ route('penjualan.axios.store') }}', {
                    _token: "{{ csrf_token() }}",
                    total: total,
                    barang: cart          
                })
            ...
            $('#kode').keypress(function (e) {
                if (e.which == 13) {                  
                    ...
                    axios.post( '{{ route('penjualan.axios.search') }}', {
                        _token: "{{ csrf_token() }}",
                        kode: kode.val()
                    })
                    .then(function(response) {                            
                        kode.val(response.data.data.id_barang);
                        nama.val(response.data.data.nama);
                        harga.val(formatRupiah(response.data.data.harga)).attr("data-harga", response.data.data.harga);
